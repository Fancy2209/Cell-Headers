...
def usage():
    print("ps3nidreader by xerpi - automated version")
    print("Usage:\n\t" + sys.argv[0] + " folder_with_prx")

# ----------------------------
# PRX/ELF Readers (unchanged)
# ----------------------------
...
def export_nids_yaml(modinfo, outdir):
    """Write one YML per library into outdir"""
    ...
    for exp in modinfo.exports:
        ...
        # sanitize filename
        filename = os.path.join(outdir, exp.name + ".yml")

        with open(filename, "w") as yf:
            yf.write("modules:\n")
            yf.write("  " + modinfo.name + ":\n")
            yf.write("    libraries:\n")
            yf.write("      " + exp.name + ":\n")
            yf.write("        functions:\n")
            ...
            for funcname, nid_formatted in functions:
                yf.write(f"          {funcname}: {nid_formatted}\n")
...
def process_file(filename, outdir):
    with open(filename, 'rb') as f:
        ...
        if elffile.header.e_ident["EI_OSABI"] == ELFOSABI_CELL_LV2:
            ...
            export_nids_yaml(modinfo, outdir)
        else:
            print(f"Skipping {filename}: not a PS3 PPU ELF")
...
if __name__ == '__main__':
    ...
    indir = sys.argv[1]
    outdir = os.path.join("db")

    for root, _, files in os.walk(indir):
        for file in files:
            if file.endswith(".prx"):
                filepath = os.path.join(root, file)
                process_file(filepath, outdir)
